...
async function scrape(_req: ScrapeRequest): Promise<ScrapeResponse> {
  // TODO: implement LinkedIn scraping via Playwright
  return { jobs: [], error: "LinkedIn scraper not yet implemented" };
}
...
server.listen(PORT, () => {
  console.log(`LinkedIn scraper listening on port ${PORT}`);
});
