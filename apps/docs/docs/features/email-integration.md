...
JobTopBob can connect to your Gmail account to automatically detect job-search-related emails in real time. New emails trigger push notifications via Google Cloud Pub/Sub, are classified by AI, and appear in the Tracking Inbox for your review.

## How it works

1. You connect your Gmail account via OAuth (read-only access)
2. Gmail sends a push notification to your server via Google Cloud Pub/Sub whenever a new email arrives
...
- An [AI provider configured](./ai-setup) (used for email classification)
...
- **Read-only access**: JobTopBob only requests the `gmail.readonly` scope — it cannot send, delete, or modify your emails
...
- **Instant revocation**: Disconnecting Gmail from Settings revokes the OAuth token, stops the watch, and deletes all stored tokens and email events
