...
// ListCompaniesParams holds all filter/pagination parameters for listing companies.
...
// ListCompaniesResult holds the paginated result.
type ListCompaniesResult struct {
	Data    []db.ListCompaniesRow `json:"data"`
	Total   int64                 `json:"total"`
	Page    int32                 `json:"page"`
	PerPage int32                 `json:"per_page"`
}

// ListCompanies returns a paginated, filtered list of companies with job counts.
func ListCompanies(ctx context.Context, q *db.Queries, userID string, p ListCompaniesParams) (ListCompaniesResult, error) {
	if p.Page < 1 {
		p.Page = 1
	}
	if p.PerPage < 1 {
	...
	filterParams := db.CountCompaniesParams{
		UserID:             userID,
		Industries:         p.Industries,
		Sizes:              p.Sizes,
		DataSources:        p.DataSources,
		EnrichmentStatuses: p.EnrichmentStatuses,
		Search:             p.Search,
	}

	total, err := q.CountCompanies(ctx, filterParams)
	if err != nil {
		return ListCompaniesResult{}, err
	}

	companies, err := q.ListCompanies(ctx, db.ListCompaniesParams{
		UserID:             userID,
		Limit:              p.PerPage,
		Offset:             offset,
		Industries:         p.Industries,
